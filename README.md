...
三样本地数据自备：覆盖 N37–N38 / E111–E113 的 ASTER 压缩瓦片、含市县两级的行政区划 shp、以及 `ggmapcn::check_geodata()` 自动下载的 GEBCO。脚本开头的路径改成自己的即可。压缩瓦片不必解包，`vsizip_tiles()` 会读归档拼出 GDAL 虚拟路径。

## 中国地图的底图

边界数据由使用者自备，用自然资源部标准地图服务（bzdt.ch.mnr.gov.cn）下载的标准地图，本仓库不分发：标准地图的效力属于下载者本人那一份。

本仓库示例以「中国省市县标准行政区划数据 审图号GS（2024）0650号」作示范，最终以使用者自备的版本为准。

《公开地图内容表示规范》（自然资规〔2023〕2 号）的相关条款：
...
`check_cn_content()` 核查曾母暗沙、黄岩岛、西沙、钓鱼岛、赤尾屿是否落在图框（含角框）内，两个三级示例均通过。这是覆盖性核查，不是合规判定：合规取决于所用边界数据、注记与送审，本技能不作担保。公开发表前请按《地图管理条例》（国务院令第664号）送审。

示例的角框未加注记，由使用者按投稿要求自行决定。要加就填 `INSET_LABEL`，并相应放宽 `IX`。

正图与附图两种做法规范都允许。定位面板尺寸小时建议放正图：角框要留出名称位置就不能太小，框一大又要把窗口继续东扩才不压陆地。实测在 45.6 mm 宽的定位框里，角框版把中国框由 46.7 mm 压到 32.9 mm、窗口东扩到约 150°E。

## 需要自己准备的数据

仓库不含 shapefile 和栅格。边界数据各地区来源不同，图注需写明出处与版本；DEM 体积大，且已有公开服务。
